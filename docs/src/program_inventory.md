...
# Execution Examples & Direct Code Links

## build_mart
File: `scripts/build_mart.py`
Run:
```bash
...
Run:
```bash
MMX_INFERENCE_BACKEND=REFERENCE python scripts/train_model.py
```
...
Run:
```bash
python scripts/recommend.py --period-start 2026-03-01 --period-end 2026-03-31 --total-budget 10000000
```
...
| Component | Path |
|-----------|------|
| SEM Model | src/mmx/engine/pymc_sem.py |
| Adstock | src/mmx/engine/adstock.py |
| Saturation | src/mmx/engine/saturation.py |
| Posterior Inference | src/mmx/engine/inference.py |
| Objective Function | src/mmx/optimization/objective.py |
| Solver | src/mmx/optimization/solver.py |
| Constraints | src/mmx/optimization/constraints.py |
| Data Quality Gate | scripts/build_mart.py |
| Governance UI | apps/dashboard/pages/p5_governance_monitoring.py |
